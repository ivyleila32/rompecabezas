...
var instrucciones = [
  'Usar las flechas para mover las piezas',
  'El rompecabeza debe quedar como el modelo'
];

// Bandera que indica si esta mezclando o no
var mezclando = false;
...
function ingresarMovimientos(direccion) {
...
function chequearSiGano() {
  ...
  for (i = 0; i < grilla.length; i++) {
    for (j = 0; j < grilla.length; j++) {
      if (numeroAnterior < grilla[i][j]) {
        numeroAnterior = grilla[i][j];
      }
      else {
        return false;
      }
...
function intercambiarPosicionesGrilla(filaPos1, columnaPos1, filaPos2, columnaPos2) {
  var varAux = grilla[filaPos1][columnaPos1];
  grilla[filaPos1][columnaPos1] = grilla[filaPos2][columnaPos2];
  grilla[filaPos2][columnaPos2] = varAux;
...
function volverJugar() {
  var btn = document.getElementById("volverJgr");
  btn.onclick = function () {
    mezclarPiezas(30);
    resetContMonvimientos();
    actualizarInfo();
...
function cambiarBtn() {
  document.getElementById("btnIniciar").classList.toggle("ocultar");
  document.getElementById("volverJgr").classList.toggle("ocultar");
}
...
function resetContMonvimientos() {
  contMovimientos = 0;
}
function actualizarInfo() {
  document.getElementById("puntaje").innerHTML = contMovimientos;
}
function mostrarHighScores() {
